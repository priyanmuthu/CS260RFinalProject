...
def check_val_acc(model, world_size, dataset_name, rank, dataloader, criterion, optimizer, num_epochs=25, is_inception=False):
    since = time.time()
    ...
    sync_epoch_loss = reduce_tensor(epoch_loss_tensor, world_size)
    ...
    sync_epoch_acc = reduce_tensor(epoch_acc_tensor, world_size)
...
def train_model(model, world_size, dataset_name, rank, dataloaders, criterion, optimizer, num_epochs=25, is_inception=False, start_epoch=0):
    since = time.time()
...
def main(rank, world_size, data_dir, dataset_name, num_classes):
    ...
    model_ft, input_size = initialize_model(model_name, num_classes, feature_extract, use_pretrained=False)
    model_ft = model_ft.to(rank)
    # wrap the model with DDP
    # device_ids tell DDP where is your model
    # output_device tells DDP where to output, in our case, it is rank
    # find_unused_parameters=True instructs DDP to find unused output of the forward() function of any module in the model
    model_ft = DDP(model_ft, device_ids=[rank], output_device=rank, find_unused_parameters=False)
    ...
    image_samplers = {x: DistributedSampler(image_datasets[x], num_replicas=world_size, rank=rank, shuffle=True, drop_last=False) for x in ['train', 'val']}
    # Create training and validation dataloaders
    dataloaders_dict = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=batch_size, pin_memory=True, shuffle=False, num_workers=4, drop_last=False, sampler=image_samplers[x], persistent_workers=True) for x in ['train', 'val']}
    ...
    # Train and evaluate
    model_ft, hist = train_model(model_ft, world_size, dataset_name, rank, dataloaders_dict, criterion, optimizer_ft, num_epochs=num_epochs, is_inception=(model_name=="inception"), start_epoch=0)
...
if __name__ == '__main__':
    ...
    mp.spawn(
        main,
        args=(world_size,data_dir,dataset_name,num_classes),
        nprocs=world_size
    )
